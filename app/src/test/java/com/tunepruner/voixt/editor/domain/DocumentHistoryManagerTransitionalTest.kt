...
import com.tunepruner.voixt.editor.editorscreen.domain.DocumentHistoryManagerTransitional
import com.tunepruner.voixt.editor.editorscreen.model.toDomainStringTransitional
...
internal class DocumentHistoryManagerTransitionalTest {
    private val _sut = DocumentHistoryManagerTransitional()
    ...
    fun `add word and test against backup`() = runTest {
        _sut.initialize(DUMMY_TEXT.split(' ').map { it.toDomainStringTransitional() })
        _sut.add(listOf("ADDED", "WORDS").map { it.toDomainStringTransitional() }, 5)
    ...
    fun `add words and test size of list`() = runTest {
        ...
        _sut.add(wordsToAdd.map { it.toDomainStringTransitional() }, 5)
    ...
    fun `add word, remove word, then undo, and test against backup`() = runTest {
        _sut.initialize(DUMMY_TEXT.split(' ').map { it.toDomainStringTransitional() })
        _sut.add(listOf("ADDED", "WORDS").map { it.toDomainStringTransitional() }, 5)
        _sut.remove(5, listOf("ADDED", "WORDS").map { it.toDomainStringTransitional() })
        _sut.undo()
        assert(_sut.compileListFromEditStack().size == _sut.backupTextBodyState.value.size)
    }
    ...
    fun `undo on empty document does not crash`() = runTest {
        _sut.initialize(ArrayList<String>().map { it.toDomainStringTransitional() })
        _sut.undo()
        assert(_sut.compileListFromEditStack().size == _sut.backupTextBodyState.value.size)
    }
    ...
    fun `expected list sizes after undo once`() = runTest {
        ...
        _sut.add(arrayListOf("One").map { it.toDomainStringTransitional() })
        _sut.add(arrayListOf("Two").map { it.toDomainStringTransitional() })
        _sut.add(arrayListOf("Three").map { it.toDomainStringTransitional() })
        _sut.undo()
    ...
    fun `Testing with concatenation 1`() = runTest {
        ...
        _sut.add(arrayListOf("thr33", "f4or", "fiv5").map { it.toDomainStringTransitional() })
